...
def create_video_from_images(image_folder, output_video_path, frame_rate=25):
    ...
    image_files = [f for f in os.listdir(image_folder) 
                   if os.path.splitext(f)[1] in valid_extensions]
    image_files.sort()  # 按字母顺序排序文件
    print(image_files)
    if not image_files:
        raise ValueError("在指定文件夹中未找到有效的图片文件。")
    ...
    image_sequence = [cv2.cvtColor(cv2.imread(img), cv2.COLOR_BGR2RGB) for img in tqdm(image_paths, desc="Loading images")]
