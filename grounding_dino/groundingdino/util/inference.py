...
def predict(
    ...
    max_prob = prediction_logits.max(dim=1)[0] # max_prob.shape = (n,)
    if box_num_anchor is not None:
        topk_indices = max_prob.topk(box_num_anchor, largest=True).indices # use the box_num_anchor to filter out the boxes
        mask = torch.zeros_like(max_prob, dtype=torch.bool)
        mask[topk_indices] = True
    else:
        mask = max_prob > box_threshold # use the box_threshold to filter out the boxes
