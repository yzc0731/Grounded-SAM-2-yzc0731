...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state_with_image(
        ...
        """Initialize an inference state with a image."""
        compute_device = self.device  # device of the model
        inference_state = {}

        # normalize the images
        images = images / 255.0
        img_mean = torch.tensor((0.485, 0.456, 0.406), dtype=torch.float32, device=images.device).view(3, 1, 1)
        ...
        # Warm up the visual backbone and cache the image feature on frame 0
        self._get_image_feature(inference_state, frame_idx=0, batch_size=1)
        # this may be not important because the image feature of frame_idx will be calculated when single frame inference
        return inference_state
    
    @torch.inference_mode()
    def add_state_with_new_image(
        self, 
        inference_state,
        images,
    ):
    ...
    def propagate_in_video_preflight(self, inference_state):
        # merge the temporary outputs into the output_dict
        """
    ...
    def propagate_in_video(
        ...
        # TODO: I am still confused with how to propagate the mask in the video
        ...
        # # This part can be omitted, if the default is from 0 to the end of the video
        # # set start index, end index
        # # prepare for the processing order
        if start_frame_idx is None:
            # default: start from the earliest frame with input points
            start_frame_idx = min(output_dict["cond_frame_outputs"]) # 0
        ...
        # calculate the processing order
        if reverse:
        ...
        processing_order = range(start_frame_idx, inference_state["num_frames"])
        ...
        for frame_idx in processing_order:
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"]:
                ...
                # HERE use the output of the model to update the output dict. 
                # So it means that if we store the previous output mask into this output dict,
                # we can use it in next iteration. 
